...
bool setPositionMode(uint32_t id, bool doSerial = false){
  return bool(getInfo(id, position_mode_command, doSerial) == 1);
}

bool setVelocityMode(uint32_t id, bool doSerial = false){
  return bool(getInfo(id, velocity_mode_command, doSerial) == 1);
}
